package Java1课本示例代码.Chapter04.Example4_23;

import Java1课本示例代码.Chapter04.Example4_22.Rational;
...
public class Example4_23 {
    public static void main(String[] args) {
        ...
        item.setNumerator(2);
        item.setDenomerator(1);
        while (k <= n) {
            sum = sum.add(item);
            k++;
            int fenzi = item.getNumerator();
            int fenmu = item.getDenomerator();
            item.setNumerator(fenzi + fenmu);
            item.setDenomerator(fenzi);
        }
        a = sum.getNumerator();
        b = sum.getDenomerator();
